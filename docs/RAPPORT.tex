...
\title{\textbf{Voyage Assistant}\\[0.3cm]
\large Assistant IA conversationnel pour le transport\\
\normalsize Rapport de projet --- SAE2 BUT3}
\author{}
\date{}
...
\centering
\vspace*{2cm}
{\Huge\bfseries Voyage Assistant}\\[0.5cm]
{\Large Assistant IA conversationnel pour le transport}\\[0.3cm]
{\large Rapport de projet --- SAE2 BUT3}\\[1.5cm]

\begin{tabular}{>{\bfseries}l l}
Ako Christian & IA conversationnelle \& RAG, déploiement \\[0.2cm]
Carld Similien & Back-end \& intégration (API, base de données) \\[0.2cm]
Noé Cervera & Modèles \& évaluation (embeddings, reranker, NER) \\[0.2cm]
Dhanoush Kessavane & Front-end, design \& documentation \\
\end{tabular}\\[1.5cm]

Encadrement : \textbf{Bilal Faye} (LIPN, CNRS UMR 7030)\\[0.3cm]
Promotion 2025--2026\\[2cm]

Application en ligne : \url{https://optimalako-voyage-assistant.hf.space}\\[0.2cm]
Code source : \url{https://github.com/optmlako2004/chatbot}
...
ligne} et accessible publiquement, et son interface est \textbf{multilingue}
...
Multilingue (bonus) & Interface et assistant en français, anglais et espagnol \\
...
\begin{itemize}[nosep]
  \item \textbf{Frontend} (\code{frontend/}) : React 18 (sans étape de build, via Babel
        navigateur), CSS personnalisé avec thèmes clair/sombre et design responsive,
        \textbf{Web Speech API} pour la voix, et un moteur de \textbf{traduction}
        français / anglais / espagnol.
...
liaisons réelles, tous modes confondus), \code{Trajet} (occurrence datée d'une route),
\code{Billet}, \code{Reclamation}, ainsi que \code{ChatSession} et \code{ChatMessage}
pour la \textbf{persistance des conversations}.
...
sert qu'à \textbf{rédiger} la réponse, jamais à exécuter une action sensible. Une
consigne de langue lui est ajoutée pour qu'il réponde dans la langue choisie par
l'utilisateur.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Interface multilingue} français / anglais / espagnol, avec sélecteur de
        langue ; l'assistant répond également dans la langue choisie.
...
Carld Similien & Développement back-end (API FastAPI, base de données) et travail
d'intégration entre les modules. \\
Noé Cervera & Mise au point et évaluation des modèles (embeddings, reranking, NER),
qualité des réponses. \\
Dhanoush Kessavane & Développement du front-end, conception graphique, tests de
l'application et rédaction de la documentation. \\
\bottomrule
\end{tabular}
\end{center}
...
sessions, option vocale, interface multilingue, recherche web enrichie d'APIs temps
réel, et \textbf{déploiement en ligne effectif}. Au-delà de la simple intégration d'un
modèle, le projet démontre une architecture où l'IA s'appuie sur des \textbf{sources de
vérité} pour rester factuelle et utile.
